...
Twenty-three migrations, run in order:
...
18. `0018_bill_item_discount_and_order.sql` — turns out "don't touch the schema" from `0017` was premature: without recording the percent itself, reopening a discounted item for edit always showed 0%, because only the discount's *effect* on the amount survived, not the value. Adds `bill_items.discount_pct`; `add_bill_item_by_token`/`update_bill_item_by_token` both gain a `p_discount_pct` param (both dropped and recreated, signatures changed). Also adds `order by i.created_at, i.id` to `get_bill_by_token`'s items query, fixing the other half of a "items reorder themselves when you edit one" bug — the other half being the frontend switching from delete-then-recreate to actually calling `update_bill_item_by_token` for a same-person-count edit, so the row (and its position) doesn't change at all.
19. `0019_notes_and_shared_items.sql` — adds `bill_items.note`/`split_group_id` (nullable text/uuid) and `expenses.note`. `add_bill_item_by_token`/`update_bill_item_by_token`/`add_expense_by_token`/`update_expense_by_token` all gain trailing `p_note` params (the two bill-item functions also gain `p_split_group_id`) — all four dropped and recreated, signatures changed. `split_group_id` is deliberately not a proper items/shares join table: `bill_items` has been one-row-per-person since `0005`, and every total/settle calculation already reads it that way — rows from one "Split Equally" entry across more than one person just share a `split_group_id` so the frontend can fold them into a single display row, while the underlying data (and every existing calculation) is untouched. "Duplicate Amount" rows are never grouped.
20. `0020_trip_linked_bills.sql` — lets a trip have its own "Add a Bill" itemized bills, without inventing a second balance-math system. Adds `bills.trip_id`/`linked_expense_id`, `expenses.linked_bill_id`, and `bill_members.trip_member_id`. New `create_trip_bill_by_token(p_trip_token, p_name, p_currency, p_payer_trip_member_id default null)` creates a normal bill whose members are mirrored 1:1 from the trip's own `trip_members` (each getting a `trip_member_id` back-reference) instead of a client-typed name list. New `link_trip_bill_expense(p_trip_token, p_bill_id, p_expense_id)` records the two-way pointer once a bill's total has been mirrored into a trip expense (via the *existing*, unmodified `add_expense_by_token`/`update_expense_by_token` with `split_type = 'exact'` — `_recompute_expense_splits` from `0009` already validates that an exact split sums to the total, so this needed no new validation logic at all). `get_trip_by_token` now also returns a `bills` array (with each bill's own `share_token`, unlike `get_bill_by_token` which strips its own) so the frontend can open a bill-backed expense's itemized view in one round trip. `delete_bill_by_token` now also deletes the mirrored expense when `linked_expense_id` is set, so deleting a trip-linked bill doesn't leave a dangling "ghost" expense behind.
21. `0021_bill_trip_token.sql` — fixes a real bug from `0020`: syncing a trip-linked bill's total into its trip needs the *parent trip's* own share token, which a bill has no way to read from its own data. `get_bill_by_token` (same signature) now also returns it directly as a top-level `trip_share_token` key whenever `bills.trip_id` is set — mirrors `0020`'s `get_trip_by_token` exposing each linked bill's token the other direction, same trust model. Before this, the frontend resolved the parent trip's token from whatever trip record happened to already be loaded in memory that session, which silently failed (no error — the sync function just no-opped) whenever a trip-linked bill was opened any way other than tapping its row inside the trip's own expense list. **Its effect is undone by `0023` below** — kept in the migrations folder unedited, per the "never edit a migration after it's written" rule, even though the feature it supported no longer exists.
22. `0022_expense_date.sql` — adds `expenses.expense_date date not null default current_date` plus an `idx_expenses_trip_date (trip_id, expense_date desc)` index — a user-editable "date this actually happened" field, separate from `created_at` (still a pure bookkeeping timestamp). `add_expense_by_token`/`update_expense_by_token` both gain a trailing `p_expense_date date default current_date` param (both dropped and recreated, signatures changed). Validated with pglite (4 checks): default-to-today, an explicit backdated date on create, a date change via update, and round-tripping through `get_trip_by_token`.
23. `0023_remove_trip_linked_bills.sql` — completely reverts `0020`/`0021`'s "Add a Bill inside a Trip" feature, data included, per explicit request rather than leaving it as dead schema. Order matters here: deletes every trip-linked bill (and its mirrored expense) first, while the linking columns are still around to identify what to delete; then drops `create_trip_bill_by_token`/`link_trip_bill_expense` (the two RPCs that only existed for this feature); then reverts `get_trip_by_token`/`get_bill_by_token`/`delete_bill_by_token` back to their pre-`0020`/pre-`0021` bodies (same signatures throughout, no drops needed); then drops the four linking columns (`bills.trip_id`/`linked_expense_id`, `expenses.linked_bill_id`, `bill_members.trip_member_id`) and their index. Validated with pglite (14 checks): a trip-linked bill + its mirrored expense exist before cleanup and are both gone after (including cascaded `bill_items`); an unrelated ordinary expense survives untouched; all four columns and both functions are confirmed gone via `information_schema`/`pg_proc`; `get_bill_by_token`/`get_trip_by_token` still work correctly and no longer return the removed keys.
