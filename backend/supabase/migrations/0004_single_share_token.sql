-- Settlr — collapse edit_token/view_token into one share_token
--
-- The frontend no longer distinguishes an "editor" link from a
-- "viewer" link — every guest who has a trip's link gets full
-- edit access. This migration drops the now-unused view_token
-- column, renames edit_token to share_token, and rewrites every
-- guest RPC function that referenced the old columns or the
-- editor/viewer role concept.

alter table public.trips drop column view_token;
alter table public.trips rename column edit_token to share_token;
comment on column public.trips.share_token is 'Grants full guest read+write access to this trip. There is no separate read-only link.';
...
-- Grants/revokes on these functions are unchanged from 0003 (same
-- names + signatures) — Postgres keeps existing GRANTs across a
-- CREATE OR REPLACE, so nothing to re-run here.
