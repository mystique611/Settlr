...
- **"Add a Bill" inside a Trip is completely removed — UI, JS, and data.** It never quite worked as a clean hybrid feature (see the sync bug below), and rather than keep patching a feature being deleted, the decision was to remove it outright: the "Add a Bill" button, its bill-creation screen, and every function that only existed to support it (`openAddTripBill`, `createTripBill`, `openTripBillFromExpense`, `backFromBillDashboard`, `syncTripLinkedBillExpense`) are gone from the frontend. New migration `0023_remove_trip_linked_bills.sql` deletes any bills/mirrored expenses that were already created via this feature, drops the two RPCs that only existed for it (`create_trip_bill_by_token`, `link_trip_bill_expense`), reverts `get_trip_by_token`/`get_bill_by_token`/`delete_bill_by_token` back to their pre-feature shape, and drops the four linking columns (`bills.trip_id`/`linked_expense_id`, `expenses.linked_bill_id`, `bill_members.trip_member_id`). This is irreversible and was confirmed with the user before running. Validated with pglite: 14 checks covering pre/post cleanup state, cascade deletes, an unrelated ordinary expense surviving untouched, and all four columns/two functions confirmed gone.
- **Trip expenses now have a user-editable date** (`expense_date`, distinct from the existing `created_at` bookkeeping timestamp) — defaults to today on Add Expense, editable to backdate. New migration `0022_expense_date.sql` adds the column plus a trailing `p_expense_date` param on `add_expense_by_token`/`update_expense_by_token`. Validated with pglite (4 checks): default-to-today, explicit backdating, updating an existing expense's date, and round-tripping through `get_trip_by_token`.
- **Trip dashboard groups expenses under date headings** ("Today", "Yesterday", or a weekday+date for anything older), newest date first, and shows an overlapping avatar stack (capped at 4 + a "+N" badge) for everyone in that expense's split — so who's involved is visible without opening the row.
- **Trip Stats gained category and date filters** ("All dates", Today, Last 7/30 days, This month), and the old "Export as CSV" button was removed from the Stats screen in favor of the richer export below.
- **Trip dashboard: export as image or CSV**, both now including a separate Settle Up section (not just the raw expense list) so either file is a complete standalone record. The image export reuses the same canvas-drawing approach as Bill Split's "Save as Image" (no external library) with Settle Up drawn first, then the date-grouped transactions, then a total. The CSV gets a blank-line-separated "Settle Up" section after the expense rows, with proper quoting for descriptions containing commas.
- **Bill Split Settle Up: the "Payable to X" summary now sits at the top of the page**, above the itemized breakdown, matching where it already was in the Save as Image export.
- **Bill Split Settle Up: each person's itemized subtotal now shows both incl-tax and excl-tax figures** — the incl-tax total (their fair share of discount, service, GST, and tip) as a bold rounded badge, with the pre-tax total in smaller text right below in brackets ("excl. tax and GST") whenever the two actually differ. Applied to both the on-screen breakdown and the Save as Image export. `computeBillTotalsFor()` now also returns `perPersonExclTax` alongside the existing `perPerson`.
- **Bill Split dashboard item rows show the payable person's avatar instead of typing out their name** — "Payable by [avatar]" for a single person, "Shared by [avatar][avatar]..." for a grouped equal split, reusing the same mate-avatar stack introduced for the trip dashboard's date-grouped rows.
- Verified the full batch with a JS syntax check after every edit round, plus a final grep confirming zero leftover references to any removed "Add a Bill" identifier (`tripbillcreate`, `addTripBillFab`, `openAddTripBill`, `createTripBill`, `openTripBillFromExpense`, `backFromBillDashboard`, `syncTripLinkedBillExpense`, `tripToken`, `linkedBillId`, `linkedExpenseId`, `memberTripMemberIds`).
