...
- The trip dashboard groups expenses under date headings (Today / Yesterday / weekday+date, newest first) and shows an overlapping avatar stack — on its own row below "Paid by X · split label" — for everyone involved in each expense's split. Editing an expense returns you to All Expenses afterward; adding a new one returns to the dashboard
- Trip Stats shows both a "By Category" and a "By Date" breakdown, filterable by trip mate (anyone who paid or was in the split) and by date — a preset range (today / last 7 / last 30 days / this month) or one exact calendar date
- Export a trip's transaction list as a PNG or CSV — two buttons below Settle Up/Stats on the trip dashboard — both include a separate Settle Up section (who owes whom), not just the raw expense list
